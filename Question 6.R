...
str(data) 
#here we change some values to facts such as Sex, Class (because of number) and Age
data$Sex <- as.factor(data$Sex)
data$Class <- as.factor(data$Class)
data$Age <- as.factor(data$Age)
## This for clarify whether 0 = dead and 1= alive  
data$Survived <- ifelse(test=data$Survived == 0, yes="Dead", no="Alive")
#Then we conver those values into factors 
...
xtabs(~ probability.of.Survived + Sex, data=predicted.data)
...
logistic1 <- glm(Survived ~ ., data=data, family="binomial")
summary(logistic)
...
predicted.data <- data.frame(
  probability.of.Survived=logistic1$fitted.values,
...
ggplot(data=predicted.data, aes(x=rank, y=probability.of.Survived)) +
  ...
  xlab("Index") +
  ylab("Predicted probability of Survived")
